    ...
    if choice == 'User':
        ...
        if st.button('Login'):
            if ad_user == 'nithinma' and ad_password == 'nithin123':
                ...
                fig = px.pie(df, values='count', names=labels, title='Predicted Field according to the Skills')
                st.plotly_chart(fig)
                ...
                fig = px.pie(df, values='count', names=labels, title="Pie-Chart📈 for User's👨‍💻 Experienced Level")
                st.plotly_chart(fig)
